package com.soaring.widget.wheelview;
...
public class WeightWheelView extends View {
	...
	public WeightWheelView(Context context, AttributeSet attrs) {
		super(context, attrs);
	 ...
	 *            刻度盘精度：<br>
	 */
	public void initViewParam(float defaultValue, float maxValue, int model) {
		switch (model) {
		case MOD_TYPE_HALF:
			mModType = MOD_TYPE_HALF;
			mLineDivider = ITEM_HALF_DIVIDER;
			mValue = (int) (defaultValue * 2);
			mMaxValue = (int) (maxValue * 2);
			break;
		case MOD_TYPE_ONE:
				...
				if ((mValue + i) % mModType == 0) {
					...
					if (mValue + i <= mMaxValue) {
						...
						case MOD_TYPE_HALF:
							...
							break;
						case MOD_TYPE_ONE:
							canvas.drawText(String.valueOf(mValue + i), xPosition - (textWidth * numSize / 2), getHeight() - textWidth,
									textPaint);
							break;
