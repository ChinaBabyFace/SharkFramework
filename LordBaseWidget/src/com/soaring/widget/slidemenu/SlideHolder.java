 ...
 *
 */
package com.soaring.widget.slidemenu;
 ...
 * <!-- 在此添加修改列表，参考第一行内容 -->
 * <tr><td>1</td><td>Renyuxiang</td><td>2015-1-13 上午11:23:47</td><td>建立类型</td></tr>
 * 
 * </table>
 * @version 1.0
 * @author Renyuxiang
...
public class SlideHolder extends FrameLayout {
	...
	protected final static int MODE_READY_TO_SLIDE = 0;
	protected final static int MODE_BEING_SLIDED = 1;
	protected final static int MODE_SLIDE_FINISHED = 2;
	...
	private int mMode = MODE_READY_TO_SLIDE;
	...
	public boolean isOpened() {
		return mAlwaysOpened || mMode == MODE_SLIDE_FINISHED;
	}
	...
	public boolean open() {
		if(isOpened() || mMode == MODE_BEING_SLIDED) {
			return false;
		}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		} else if(mMode == MODE_SLIDE_FINISHED) {
			mOffset = mDirection*menuWidth;
		} else if(mMode == MODE_READY_TO_SLIDE) {
			mOffset = 0;
		}
	...
	protected void dispatchDraw(Canvas canvas) {
		try {
			if(mMode == MODE_BEING_SLIDED) {
				...
				if(!mAlwaysOpened && mMode == MODE_READY_TO_SLIDE) {
		        	mMenuView.setVisibility(View.GONE);
		        }
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		if(((!mEnabled || !mInterceptTouch) && mMode == MODE_READY_TO_SLIDE) || mAlwaysOpened) {
			return super.dispatchTouchEvent(ev);
		}
		
		if(mMode != MODE_SLIDE_FINISHED) {
			onTouchEvent(ev);
	...
	private boolean handleTouchEvent(MotionEvent ev) {
		...
		if(ev.getAction() == MotionEvent.ACTION_MOVE) {
			...
			if((mDirection*diff > 50 && mMode == MODE_READY_TO_SLIDE) || (mDirection*diff < -50 && mMode == MODE_SLIDE_FINISHED)) {
				mHistoricalX = (int) x;
				
				initSlideMode();
			} else if(mMode == MODE_BEING_SLIDED) {
				mOffset += diff;
		...
		return mMode == MODE_BEING_SLIDED;
	}
