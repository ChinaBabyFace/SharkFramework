...
package com.soaring.widget.pulltorefresh.gridview;
...
public class StaggeredGridView
	 ...
	 *
	 * - mFirstPosition is the adapter position of the View returned by getChildAt(0). - Any child index can be translated to an adapter position by adding mFirstPosition. - Any adapter position can be translated to a child index
	 * by subtracting mFirstPosition. - Views for items in the range [mFirstPosition, mFirstPosition + getChildCount()) are currently attached to the grid as children. All other adapter positions do not have active views.
	 *
	 ...
	 *
	 * LayoutRecords are used to track data about a particular item's layout after the associated view has been removed. These let positioning and the choice of column for an item remain consistent even though the rules for
	 * filling content up vs. filling down vary.
	 *
	 * Whenever layout parameters for a known LayoutRecord change, other LayoutRecords before or after it may need to be invalidated. e.g. if the item's height or the number of columns it spans changes, all bets for other items in
	 * the same direction are off since the cached information no longer applies.
	 */
	...
	public void setColumnCount(int colCount) {
		if (colCount < 1 && colCount != COLUMN_COUNT_AUTO) { throw new IllegalArgumentException("Column count must be at least 1 - received " + colCount); }
	...
	final LayoutRecord getNextRecordUp(int position, int span) {
		...
		if (rec == null) {
			...
			rec.span = span;
			mLayoutRecords.put(position, rec);
		} else if (rec.span != span) { throw new IllegalStateException("Invalid LayoutRecord! Record had span=" + rec.span + " but caller requested span=" + span + " for position=" + position); }
		int targetCol = -1;
		int bottomMost = Integer.MIN_VALUE;
	...
	private class RecycleBin {
		...
		public void setViewTypeCount(int viewTypeCount) {
			if (viewTypeCount < 1) { throw new IllegalArgumentException("Must have at least one view type (" + viewTypeCount + " types reported)"); }
