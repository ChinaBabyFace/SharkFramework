...
//		final String path = cursor.getString(GalleryPickerActivity.DATA_INDEX);
		final long id = cursor.getLong(PhotoPickerActivity.ID_INDEX);
		...
		if (width <= 0) {
			iv.getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
				...
				public void onGlobalLayout() {
					...
					Picasso.with(context).load(uri).placeholder(R.drawable.photo_picker_dfault_img).resize(iv.getWidth(), iv.getHeight()).centerCrop().into(iv);
				}
			});
		} else {
			Picasso.with(context).load(uri).placeholder(R.drawable.photo_picker_dfault_img).resize(iv.getWidth(), iv.getHeight()).centerCrop().into(iv);
	...
	public boolean isItemChecked(int position) {
		if (mCheckStates != null) { return mCheckStates.get(position); }
