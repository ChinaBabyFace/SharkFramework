...
public class MultiSelectionUtil {
	...
	public static Controller attachMultiSelectionController(final AbsListView listView, final ActionBarActivity activity, final MultiChoiceModeListener listener) {
		return Controller.attach(listView, activity, listener);
	}

	public static class Controller
			implements ActionMode.Callback, AdapterView.OnItemClickListener, AdapterView.OnItemLongClickListener {
		...
		public void tryRestoreInstanceState() {
			if (mTempIdsToCheckOnRestore == null || mListView.getAdapter() == null) { return; }
		...
		public boolean saveInstanceState(Bundle outBundle) {
			...
			if (mActionMode != null && mListView.getAdapter().hasStableIds()) {
				final boolean g = PhotoPickerActivity.isGingerbread();
				long[] checkedIds;
				if (g) {
	...
	public interface MultiChoiceModeListener
		...
		/**
		 * @see AbsListView.MultiChoiceModeListener#onItemCheckedStateChanged(android.view.ActionMode,
		 *      int, long, boolean)
		 */
