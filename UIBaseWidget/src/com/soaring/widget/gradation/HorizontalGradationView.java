/**
 * WheelView.java 2014-12-19
 * 
 * 天津云翔联动科技有限公司(c) 1995 - 2014 。
 * http://www.soaring-cloud.com.cn
 *
 */
package com.soaring.widget.gradation;
 ...
 * <!-- 在此添加修改列表，参考第一行内容 -->
 * <tr><td>1</td><td>Renyuxiang</td><td>2015-3-27 上午9:53:29</td><td>建立类型</td></tr>
 * 
 * </table>
 * @version 1.0
 * @author Renyuxiang
 * @since 1.0
 */
public class HorizontalGradationView
		extends View {
	...
	public static final int GO_LEFT = 11;
	public static final int GO_RIGHT = 22;
	...
	public HorizontalGradationView(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	private GradientDrawable createBackground() {
		...
		int strokeColor = Color.parseColor("#FBC6CF");// 边框颜色
	...
	private void drawWheel(Canvas canvas) {
		Drawable wheel = getResources().getDrawable(R.drawable.horizontal_gradation_background);
			...
			if (xPosition + getPaddingRight() < mWidth) {
				if ((mValue + i) % mModType == 0) {
					...
					if (mValue + i <= mMaxValue) {
						switch (mModType) {
						case MOD_TYPE_HALF:
							canvas.drawText(String.valueOf((mValue + i) / 2), countLeftStart(mValue + i, xPosition, textWidth), getHeight()
									- textWidth, textPaint);
							break;
						case MOD_TYPE_ONE:
	...
	public boolean onClickEvent(int event) {
		if (mVelocityTracker == null) {
			mVelocityTracker = VelocityTracker.obtain();
		}
		switch (event) {
