 ...
 * <!-- 在此添加修改列表，参考第一行内容 -->
 * <tr><td>1</td><td>Renyuxiang</td><td>2015-1-6 下午8:11:33</td><td>建立类型</td></tr>
 * 
 * </table>
 * @version 1.0
 * @author Renyuxiang
 * @since 1.0
 */
public class SoaringCloudTagView
		extends FrameLayout
	...
	private int[] cr = { Color.BLUE, Color.CYAN, Color.DKGRAY, Color.GREEN, Color.LTGRAY, Color.BLACK, Color.MAGENTA, Color.RED, Color.YELLOW };
	...
	public SoaringCloudTagView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public SoaringCloudTagView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public SoaringCloudTagView(Context context) {
		super(context);
		init();
	}
	...
	public boolean setKeywords(String[] keywords) {
		...
		if (keywords != null && keywords.length > 0) {
			int count = keywords.length > MAX ? MAX : keywords.length;
			for (int i = 0; i < count; i++) {
				result = vecKeywords.add(keywords[i]);
			}
		}
		return result;
	}
